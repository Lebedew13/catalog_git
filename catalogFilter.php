...
$sql1 = "SELECT DISTINCT class_breed FROM `idbc_dog` WHERE `show_idbc` = ?";
$stmt = $conn->prepare($sql1);
$stmt->bind_param("i", $showid); // 'i' — это тип для целого числа
$stmt->execute();
$class = $stmt->get_result();

$sql2 = "SELECT DISTINCT type_breed FROM `idbc_dog` WHERE `show_idbc` = ?";
...
$sql3 = "SELECT DISTINCT gender FROM `idbc_dog` WHERE `show_idbc` = ?";
            ...
            <div>
                <form action="GET">
                    ...
                    <select name="filter_class">
                        ...
                        <?php foreach($class as $filter): ?>
                        <option value="<?php echo htmlspecialchars($filter['class_breed'])?>"><?php echo htmlspecialchars($filter['class_breed'])?></option>
                        <?php endforeach; ?>
                    </select> 
                
                    <select name="filter_type">
                        <option value="all">Тип</option>
                        <?php foreach($type as $filter): ?>
                        <option value="<?php echo htmlspecialchars($filter['type_breed'])?>"><?php echo htmlspecialchars($filter['type_breed'])?></option>
                        <?php endforeach; ?>
                    </select>
                
                    <select name="filter_gender">
                        <option value="all">Пол</option>
                        <?php foreach($gender as $filter): ?>
                        <option value="<?php echo htmlspecialchars($filter['gender'])?>"><?php echo htmlspecialchars($filter['gender'])?></option>
                        <?php endforeach; ?>
                    </select>
